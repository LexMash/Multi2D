    ...
    public class DeviceListener : MonoBehaviour
    {
        ...
        private void OnDeviceChanged(InputDevice device, InputDeviceChange change)
        {
            ...
            if(change == InputDeviceChange.Added)
            {
               ...
               inputUser2 = InputUser.PerformPairingWithDevice(device, inputUser);
               ...
               inputs.Add(newInput);
               newInput.Enable();
               newInput.PlayerControll.Enable();
               newInput.PlayerControll.Interact.Enable();
               newInput.PlayerControll.Interact.performed += ctx => Debug.Log($"User with id {inputUser2.id} performed interation {ctx.time}");
            }
            ...
            if(change == InputDeviceChange.Enabled && device.name == "Keyboard")
            {
               ...
               newInput.PlayerControll.Enable();
               newInput.PlayerControll.Interact.Enable();
               newInput.PlayerControll.Interact.performed += ctx => Debug.Log($"User with id {inputUser1.id} performed interation {ctx.time}");
            }
        }
